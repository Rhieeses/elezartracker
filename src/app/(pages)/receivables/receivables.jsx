...
export default function ReceivablesContent() {
	...
	const handleInputChange = (field) => (event) => {
		...
		if (field === 'paymentAmount') {
			...
			if (/^\d*\.?\d{0,2}$/.test(value)) {
				setRawPaymentAmount(value); // For tracking the raw input

				const numericValue = parseFloat(value);

				// Check if numeric value is valid and within max limit
				if (!isNaN(numericValue) && numericValue <= maxValue) {
					...
					setRawPaymentAmount(maxValue);
					setPaymentData((prev) => ({
						...prev,
						[field]: maxValue,
					}));
				} else {
					// Handle the case where input is empty or invalid
					setPaymentData((prev) => ({
						...prev,
						[field]: '',
					}));
	...
	const handleBlur = () => {
		if (paymentData.paymentAmount) {
			setPaymentData({
				...paymentData,
				paymentAmount: formatNumber(parseFloat(paymentData.paymentAmount).toFixed(2)),
			});
		}
	};
